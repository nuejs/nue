...
Run nue command on a multi-site directory or inside a site directory:
...
Create a new multi-site project from a template:

```bash
nue create multi-site
```

This extracts a downloaded template to a new directory. Issue `nue` inside that directory.
