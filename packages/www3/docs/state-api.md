...
# State API
**Nuestate** is Nue's state management library for single-page applications. It puts your application state in the URL by default, making bookmarking, sharing, and browser navigation work naturally without extra code.

Use this for SPAs and interactive dashboards. Content sites don't need state management - they use static page generation.

## Why URL-first?

Most state management treats URLs as an afterthought. You manually sync state with URLs, handle browser navigation, and write extra code for bookmarking and sharing.

Nuestate makes state live in the URL by default:

- Bookmarking works - users can bookmark any application state
- Sharing works - send someone a URL and they see what you see
...
state.view = 'users'     // URL updates automatically
...
<body>
  ...
  <script>
    state.on('id', ({ id }) => {
      ...
      this.mount(id ? 'user-detail' : 'user-list', wrap)
    })
...
Keep your business logic separate in testable JavaScript modules:

```javascript
// /admin/model/index.js
...
```html
<script>
  import { getUsers, deleteUser } from '/admin/model/index.js'
  import { state } from 'state'
</script>

<article :is="user-list">
  <table>
...
Model code is pure JavaScript with no framework dependencies. You can test it on the server side with standard testing tools. Components stay focused on UI while models handle data and business logic.
