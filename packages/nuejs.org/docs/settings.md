...
Use native CSS nesting instead of converting them to un-nested style rules that are supported in all browsers. Setting this to `true` generates a smaller CSS output, but is not supported by older browsers. Check the current [Can I Use](//caniuse.com/css-nesting) statistics for details.
