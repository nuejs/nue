...
[.note]
  The `section_component` option has been deprecated

You can turn your [sections](/docs/content-authoring.html#sections) and [grid items](/docs/reactivity.html#grid-items) into a native Web Component. For example, on the front page of this website, we have a "scroll-transition" component to help implement all the scroll-triggered CSS transitions.
...
- Files with a `.htm` suffix are treated as [client-side components](/docs/custom-components.html). Now both `.nue` and `.htm` files are treated the same.
