// ../../../node_modules/.pnpm/diff-dom@5.1.2/node_modules/diff-dom/dist/index.js
var checkElementType = function(element) {
...
var objToNode = function(objNode, insideSvg, options) {
  var node;
...
var applyDiff = function(tree, diff, options) {
  ...
  switch (action) {
    ...
    case options._const.relocateGroup:
      nodeArray = Array.apply(undefined, new Array(diff[options._const.groupLength])).map(function() {
...
var applyDOM = function(tree, diffs, options) {
  return diffs.every(function(diff) {
    return applyDiff(tree, diff, options);
  });
};
var swap = function(obj, p1, p2) {
  var tmp = obj[p1];
  obj[p1] = obj[p2];
  obj[p2] = tmp;
};
var undoDiff = function(tree, diff, options) {
...
var undoDOM = function(tree, diffs, options) {
  diffs = diffs.slice();
  diffs.reverse();
...
var getFromVirtualRoute = function(tree, route) {
  var node = tree;
  var parentNode;
  var nodeIndex;
  route = route.slice();
...
var applyVirtualDiff = function(tree, diff, options) {
  var _a;
...
var applyVirtual = function(tree, diffs, options) {
  diffs.forEach(function(diff) {
    applyVirtualDiff(tree, diff, options);
  });
  return true;
};
var nodeToObj = function(aNode, options) {
...
var findCommonSubsets = function(c1, c2, marked1, marked2) {
  ...
  var matches = Array.apply(undefined, new Array(c1Length + 1)).map(function() {
    return [];
  });
...
var makeBooleanArray = function(n, v) {
  return Array.apply(undefined, new Array(n)).map(function() {
    return v;
  });
};
...
var markSubTrees = function(oldTree, newTree) {
  ...
  var _loop_1 = function() {
    ...
    if (subset) {
      subsets.push(subset);
      var subsetArray = Array.apply(undefined, new Array(subset.length)).map(returnIndex);
